...
$totalToolsQuery = mysqli_query($connect, "SELECT COUNT(*) AS total FROM tool WHERE tool_Location = '$username'");
$totalTools = mysqli_fetch_assoc($totalToolsQuery)['total'];

$goodToolsQuery = mysqli_query($connect, "SELECT COUNT(*) AS good FROM tool WHERE tool_Condition = 'Good' AND tool_Location = '$username'");
$goodTools = mysqli_fetch_assoc($goodToolsQuery)['good'];

$badToolsQuery = mysqli_query($connect, "SELECT COUNT(*) AS bad FROM tool WHERE tool_Condition = 'Bad' AND tool_Location = '$username'");
$badTools = mysqli_fetch_assoc($badToolsQuery)['bad'];

// Fetch recent activities
$recentActivitiesQuery = mysqli_query($connect, "
    SELECT 
        h.tool_ID, h.old_condition, h.new_condition, h.update_date
        FROM tool_condition_history h
        JOIN tool t ON h.record_ID = t.record_ID
        WHERE t.tool_Location = '$username'
        ...
        <tbody>
            <?php if (count($recentActivities) > 0): ?>
                <?php foreach ($recentActivities as $activity): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($activity['tool_ID']); ?></td>
